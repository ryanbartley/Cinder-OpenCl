...
public:
	//! Returns a PlatformRef with the first available platform.
	static PlatformRef create();
	//! Returns a PlatformRef containing \a platform.
	static PlatformRef create( cl_platform_id platform );
	...
	//! Returns a PlatformRef with \a platform as the platform, containing \a devices.
	...
	cl_platform_id					getId() const { return mId; }
		void						setDevices( const std::vector<cl_device_id> &deviceIds );
